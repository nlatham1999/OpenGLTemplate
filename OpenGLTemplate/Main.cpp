...
int main()
{
    ...
    Shader lightCubeShader("lighting_light.vs", "lighting_light.fs");
...
void processInput(GLFWwindow* window)
{
    ...
    {
        mixValue += cameraSpeed / 10; // change this value accordingly (might be too slow or too fast based on system hardware)
        if (mixValue >= 1.0f)
            mixValue = 1.0f;
    }
    ...
    if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS)
        camera.StrafeUp(deltaTime);
    if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS)
        camera.StrafeDown(deltaTime);
